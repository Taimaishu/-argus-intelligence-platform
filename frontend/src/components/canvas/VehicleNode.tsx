...
export const VehicleNode = memo(({ id, data }: NodeProps<any>) => {
  const deleteNode = useCanvasStore((state) => state.deleteNode);
  return (
    <div className="relative group">
      <Handle type="target" position={Position.Top} className="!bg-red-500 !w-3 !h-3" />
      <div className="px-4 py-3 rounded-lg shadow-lg border-2 min-w-[180px] max-w-[280px] transition-all hover:scale-105 bg-gradient-to-br from-red-50 to-rose-50 dark:from-red-900/30 dark:to-rose-900/30 border-red-300 dark:border-red-700">
        ...
        <div className="flex items-center gap-2 mb-2"><div className="p-1.5 bg-red-500 rounded"><Car className="w-4 h-4 text-white" /></div><span className="text-xs font-semibold text-red-600 dark:text-red-400 uppercase">Vehicle</span></div>
